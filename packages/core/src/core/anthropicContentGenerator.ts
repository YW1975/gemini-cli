...
export class AnthropicContentGenerator implements ContentGenerator {
  ...
  async generateContent(
    ...
    try {
      ...
      if (request.config?.tools) {
        createParams.tools = await this.convertGeminiToolsToAnthropic(
  ...
  async generateContentStream(
    ...
    // Wrap non-streaming API in async generator for compatibility
    const generate = () => this.generateContent(request, userPromptId);
  ...
  private convertToAnthropicFormat(request: GenerateContentParameters): {
    ...
    if (request.config?.systemInstruction) {
      systemMessage = this.extractSystemInstruction(
    ...
    return {
      systemMessage,
      messages: this.ensureAlternatingMessages(messages),
    };
  }
  ...
  private extractSystemInstruction(systemInstruction: unknown): string {
    ...
    if (
      ...
      return (
        contentObj.parts?.map((p) => ('text' in p ? p.text : '')).join('\n') ||
        ''
      );
    }
    return '';
  ...
  /**
   * Normalize JSON Schema type values to lowercase.
   * Gemini SDK mutates tool definitions in-place, converting
   * 'object' → 'OBJECT', 'string' → 'STRING', etc.
   * Anthropic API requires lowercase type values.
   */
  private normalizeSchemaTypes(schema: unknown): unknown {
    if (schema === null || schema === undefined) return schema;
    if (Array.isArray(schema))
      return schema.map((item) => this.normalizeSchemaTypes(item));
    ...
    for (const tool of geminiTools) {
      ...
      if (toolObj.functionDeclarations) {
        for (const func of toolObj.functionDeclarations) {
          if (func.name) {
            const normalizedParams = this.normalizeSchemaTypes(func.parameters);
            anthropicTools.push({
              name: func.name,
              description: func.description || '',
              input_schema:
                (normalizedParams as Anthropic.Tool.InputSchema) || {
                  type: 'object',
                  properties: {},
                },
            });
